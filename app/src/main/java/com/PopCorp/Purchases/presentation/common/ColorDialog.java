...
public class ColorDialog extends DialogFragment implements View.OnClickListener, View.OnLongClickListener {
    ...
    public void onAttach(Activity activity) {
        ...
        /*if (!(activity instanceof ColorCallback))
            throw new IllegalStateException("ColorDialog needs to be shown from an Activity implementing ColorCallback.");*/
        mCallback = getBuilder().mCallback;
    }
    ...
    public interface ColorCallback {
        void onColorSelection(@NonNull ColorDialog dialog, @ColorInt int selectedColor);
    }
    ...
    public Dialog onCreateDialog(Bundle savedInstanceState) {
        if (getArguments() == null || !getArguments().containsKey("builder"))
            throw new IllegalStateException("ColorDialog should be created using its Builder interface.");
        generateColors();
        ...
        MaterialDialog dialog = new MaterialDialog.Builder(getActivity())
                ...
                .onPositive((dialog1, which) -> {
                    mCallback.onColorSelection(ColorDialog.this, getSelectedColor());
                    dismiss();
                })
                .onNegative((dialog1, which) -> {
                    if (isInSub()) {
                        dialog1.setActionButton(DialogAction.NEGATIVE, getBuilder().mCancelBtn);
                        isInSub(false);
                        invalidate();
                    } else {
                        dialog1.cancel();
                ...
                .onNeutral((dialog1, which) -> toggleCustom(dialog1))
                .showListener(dialog1 -> invalidateDynamicButtonColors())
                .build();
    ...
    private class ColorGridAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            child.setOnClickListener(ColorDialog.this);
            child.setOnLongClickListener(ColorDialog.this);
            return convertView;
        }
    }
    ...
    public static class Builder implements Serializable {
        ...
        protected final ColorCallback mCallback;
        @StringRes
        ...
        public Builder(@NonNull AppCompatActivity context, ColorCallback callback, @StringRes int title) {
            mContext = context;
            mCallback = callback;
            mTitle = title;
        }
    ...
    @Nullable
    public static ColorDialog findVisible(@NonNull AppCompatActivity context, @ColorChooserTag String tag) {
        Fragment frag = context.getSupportFragmentManager().findFragmentByTag(tag);
        if (frag != null && frag instanceof ColorDialog)
            return (ColorDialog) frag;
        return null;
    }

    @NonNull
    public ColorDialog show(AppCompatActivity context) {
        String tag;
        Builder builder = getBuilder();
        if (builder.mColorsTop != null)
            tag = TAG_CUSTOM;
